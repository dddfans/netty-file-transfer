...
public class TestCodeC {
	...
	public ByteBuf encode(NettyMessage msg) throws Exception {
		...
		System.out.println("sendBuf readableBytes " + sendBuf.readableBytes());
		...
		keyArray = null;
		value = null;
		System.out.println("sendBuf readableBytes ,body pos " + sendBuf.readableBytes());
		if (msg.getBody() != null) {
//			marshallingEncoder.encode(msg.getBody(), sendBuf);
//			sendBuf.writeInt(msg.getBody().length);
...
//			message.setBody(marshallingDecoder.decode(in));
	...
	public static void test2() throws Exception{
		...
		byte[] data = buf.array();
		buf2.writeBytes(buf, 0, buf.readableBytes() -1);
		System.out.println("buf2 readableBytes " + buf2.readableBytes());
		int bodyPos = Ints.fromBytes(data[8],data[9],data[10],data[11]);
		System.out.println("body pos ==> " + bodyPos);
//		buf2.setInt(bodyPos, buf2.readableBytes() - bodyPos - 4);
		NettyMessage decodeMsg = testC.decode(buf2);
		byte[] db = (byte[])decodeMsg.getBody();
