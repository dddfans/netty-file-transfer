...
public class FileSegmentRequestProcessor extends AbstractFileRequestProcessor{
	...
	public FileSegmentRequestProcessor(ConcurrentHashMap<String, FileStorage> fileTable, String baseDir, int[] nettyCommandCodes) {
	...
	public NettyCommand process(NettyCommand command) throws IOException {
		...
		NettyCommand response = new NettyCommand(command.getCmdCode(), NettyCommandType.File_Response.getCode(), false,command.getOpaque());
